...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === 'admin@konnyusu.com' && $pass === 'admin123') {
        $_SESSION['user'] = ['name' => 'Admin', 'email' => $email, 'role' => 'admin'];
        header('Location: ../admin/dashboard/dashboard.php');
        exit;
    } elseif (!empty($email) && !empty($pass)) {
        $_SESSION['user'] = ['name' => ucfirst(explode('@', $email)[0]), 'email' => $email, 'role' => 'user'];
...
<head>
  ...
  .auth-visual__cup { font-size: 6rem; display: block; margin-bottom: 1.5rem;
